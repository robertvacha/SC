...
void Inicializer::initTop() {
    ...
    while (sysnames[i]!=NULL) {
        ...
        while (strcmp(molecules[mol].name,sysnames[i])) {
            ...
            if (molecules[mol].name == NULL) {
                ...
                topDealoc(sysnames,&sysmoln, molecules);
                exit(1);
            }
        }
        ...
        for (j=0;j<sysmoln[i];j++) {
            ...
            while (molecules[mol].type[k] != -1) {
                ...
                /// count muVT particles
                if(topo->chainparam[mol].muVTmove && topo->chainparam[mol].particleTypes[1] == -1) {
                    conf->muVTpartCount++;
                }
    ...
    for(i = 0; i < maxpart; i++) {
        conf->particleStore.push_back(new Particle);
        ...
        conf->particleStore[i]->chaint              = tmp_particles[i].chaint;
        ...
        for (j=0; j<MAXCHL; j++) {
            if (topo->chainlist[i][j] >= 0) {
                ...
                if ((j+1 < MAXCHL)&&(topo->chainlist[i][j+1] >= 0))
                    conf->particleStore[k]->conlist[1] = topo->chainlist[i][j+1]; //if there is a next particle fill it to head bond
                if (j > 0)
                    conf->particleStore[k]->conlist[0] = topo->chainlist[i][j-1]; //if this is not first particle fill tail bond
                if ((j+2 < MAXCHL)&& (topo->chainlist[i][j+2] >= 0))
                    conf->particleStore[k]->conlist[3] = topo->chainlist[i][j+2]; //if there is a second next particle fill it second neighbour
                if (j > 1)
                    conf->particleStore[k]->conlist[2] = topo->chainlist[i][j-2]; //if this is not second or first particle fill second tail bond
            }
        }
...
void Inicializer::testChains() {
    if (topo->chainCount == 0) {    // no chain -> make the probability of moving them 0
        if (sim->chainprob > 0)
...
void Inicializer::initChainParams() {
...
void Inicializer::initChainLists() {
    for (int i=0;i<MAXN;i++) {
        for (int j = 0; j < MAXCHL; j++){
...
void Inicializer::initMolecules(Molecule molecules[MAXMT]) {
    for (int i=0;i<MAXMT;i++) {
        molecules[i].name = NULL;
        molecules[i].type = (int*) malloc(sizeof(long)*MAXN);
        molecules[i].switchtype = (long int*) malloc(sizeof(long)*MAXN);
        molecules[i].delta_mu = (double*) malloc(sizeof(double)*MAXN);
...
int Inicializer::topDealoc(char *sysnames[], long **sysmoln, Molecule *molecules) {
