...
double ExternalEnergyCalculator::extere2(Particle *target) {
    ...
    if (  target->pos.z < 0  ) {
        rcmz = box->z * (target->pos.z - (double)( (long)(target->pos.z - 0.5) ) );
    } else {
        rcmz = box->z * (target->pos.z - (double)( (long)(target->pos.z + 0.5) ) );
    }
    ...
    double en6;
    /* WCA repulsion */
    if (dist > param->rcutwca) {
        repenergy = 0.0;
    } else {
            en6 = pow((param->sigma / dist),6);
...
void ExternalEnergyCalculator::exter2ClosestDist() {
...
double ExternalEnergyCalculator::exter2Atre(double *ndist, int numpatch, double halfl) {
...
int ExternalEnergyCalculator::pscWall(Vector *pbeg, Vector *pend, Vector *projectdir,
                                      Vector *partdir, double *cutdist, Vector *partbeg, Vector *partend) {
    Vector vec1;
...
int ExternalEnergyCalculator::cpscWall(Vector *pbeg, Vector *pend, Vector *projectdir, Vector *partdir,
...
int ExternalEnergyCalculator::cutProjectAtWall(Vector *pextr1, Vector *pextr2, Vector *pextr3, Vector *pextr4,
...
double ExternalEnergyCalculator::exterAtre(double *ndist, int numpatch, double halfl) {
...
double ExternalEnergyCalculator::areaEightPoints(Vector *p1, Vector *p2, Vector *p3, Vector *p4,
