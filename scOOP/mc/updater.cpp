...
void Updater::initCounters() {
    long i;
    //=== Initialise counters etc. ===//
//	double pvolume;    // Volume of all particles
    /*pvolume =0.0;
    for (i=0;i < (long)conf->particleStore.size();i++) {
        if  (conf->particleStore[i].type>=0 )
            pvolume += topo->ia_params[conf->particleStore[i].type][conf->particleStore[i].type].volume;
    }*/
...
void Updater::initValues(long& next_adjust, long& next_calc, long& next_dump, long& next_frame, FILE* mf) {
...
void Updater::simulate(long nsweeps, long adjust, long paramfrq, long report) {
    ...
    initValues(next_adjust, next_calc, next_dump, next_frame, mf);
    ...
    for (sweep=1; sweep <= nsweeps; sweep++) {
        ...
        if (sweep == next_frame) {
            fprintf (mf, "%ld\n", (long)(long)conf->particleStore.size());
...
int Updater::sameCluster(long fst, long snd) {
    ...
    if ( ((topo->chainparam[conf->particleStore[fst]->chaint]).bond1c >= 0) ||
        ((topo->chainparam[conf->particleStore[fst]->chaint]).bonddc >= 0) ){
        if ( (snd == conf->particleStore[fst]->conlist[1]) || (snd == conf->particleStore[fst]->conlist[0]) ) {
          return true;
        }
    }
    if ( ((topo->chainparam[conf->particleStore[snd]->chaint]).bond1c >= 0) ||
        ((topo->chainparam[conf->particleStore[snd]->chaint]).bonddc >= 0) ){
