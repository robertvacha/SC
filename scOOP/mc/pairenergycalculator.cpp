...
double PairEnergyCalculator::angleEnergy() {
    ...
    if ((topo->chainparam[part1->chaint]).angle1c >= 0) {
        if (num2 == part1->conlist[0]) {
            ...
            if ( (geotype[0] >= SP) && (geotype[1] >= SP) )
                ...
                energy += harmonicPotential(currangle,topo->chainparam[part1->chaint].angle1eq,topo->chainparam[part1->chaint].angle1c);
            }
        } else {
            if (num2 == part1->conlist[1]) {
                ...
                /*spheres do not have this interaction*/
                    ...
                    vec2.normalise();
                    currangle = acos(DOT(vec1,vec2));
                    energy += harmonicPotential(currangle,topo->chainparam[part2->chaint].angle1eq,topo->chainparam[part2->chaint].angle1c);
                }
            }
        }
    }
    ...
    if (topo->chainparam[part1->chaint].angle2c >= 0) {
        if (num2 == part1->conlist[0]) {
            /*num1 is connected to num2 by tail*/
            if ( (geotype[0] < SP) && (geotype[1] < SP) ) {
                currangle = acos(DOT(part1->patchdir[0],part2->patchdir[0]) - DOT(part1->dir,part2->patchdir[0])  );
                energy += harmonicPotential(currangle,topo->chainparam[part1->chaint].angle2eq,topo->chainparam[part1->chaint].angle2c);
            } else {
                energy += 0.0;
            }
        } else {
            if (num2 == part1->conlist[1]) {
                ...
                if ( (geotype[0] < SP) && (geotype[1] < SP) ) {
                    ...
                    energy += harmonicPotential(currangle,topo->chainparam[part2->chaint].angle2eq,topo->chainparam[part2->chaint].angle2c);
                } else {
                    energy += 0.0;
                }
            }
...
double PairEnergyCalculator::bondEnergy() {
    ...
    if ((topo->chainparam[part1->chaint]).bond1c >= 0) {

        if (num2 == part1->conlist[1]) {
            /*num1 is connected to num2 by tail*/
            if ( (geotype[0] >= SP) && (geotype[1] >= SP) )
                energy = harmonicPotential(distcm,topo->chainparam[part1->chaint].bond1eq,topo->chainparam[part1->chaint].bond1c);
            else {
                if (geotype[0] < SP)
                    halfl =topo->ia_params[part1->type][part2->type].half_len[0];
                else
                ...
                energy = harmonicPotential(bondlength,topo->chainparam[part1->chaint].bond1eq,topo->chainparam[part1->chaint].bond1c);
            }
        } else {
            if (num2 == part1->conlist[0]) {
                /*num1 is connected to num2 by head*/
    ...
    if (topo->chainparam[part1->chaint].bond2c >= 0) {
        if (num2 == part1->conlist[2]) {
            /*num1 is connected to num2 by tail*/
            if ( (geotype[0] >= SP) && (geotype[1] >= SP) )
                energy = harmonicPotential(distcm,topo->chainparam[part1->chaint].bond2eq,topo->chainparam[part1->chaint].bond2c);
            else {
                vecbond = image(&part1->pos, &part2->pos, &box);
                bondlength = sqrt(DOT(vecbond,vecbond));
                energy = harmonicPotential(bondlength,topo->chainparam[part1->chaint].bond2eq,topo->chainparam[part1->chaint].bond2c);
            }
        } else {
            if (num2 == part1->conlist[3]) {
                /*num1 is connected to num2 by head*/
                if ( (geotype[0] >= SP) && (geotype[1] >= SP) )
                    energy = harmonicPotential(distcm,topo->chainparam[part1->chaint].bond2eq,topo->chainparam[part1->chaint].bond2c);
                else {
    ...
    if ((topo->chainparam[part1->chaint]).bonddc > 0) {

        if (num2 == part1->conlist[1]) {
            /*num1 is connected to num2 by tail*/
            if ( (geotype[0] >= SP) && (geotype[1] >= SP) )
                energy = harmonicPotential(distcm,topo->chainparam[part1->chaint].bonddeq,topo->chainparam[part1->chaint].bonddc);
            else {
                if (geotype[0] < SP)
                    halfl =topo->ia_params[part1->type][part2->type].half_len[0];
                else
                ...
                if (geotype[1] < SP)
                    ...
                    halfl = 0.0;
                vec2.x = part2->pos.x + part2->dir.x * (halfl + topo->chainparam[part1->chaint].bonddeq) / box.x ;
                vec2.y = part2->pos.y + part2->dir.y * (halfl + topo->chainparam[part1->chaint].bonddeq) / box.y ;
                vec2.z = part2->pos.z + part2->dir.z * (halfl + topo->chainparam[part1->chaint].bonddeq) / box.z ;
                vecbond = image(&vec1, &vec2, &box);
                bondlength = sqrt(DOT(vecbond,vecbond));
                energy = harmonicPotential(bondlength,0.0,topo->chainparam[part1->chaint].bonddc);
            }
        } else {
            if (num2 == part1->conlist[0]) {
                /*num1 is connected to num2 by head*/
                if ( (geotype[0] >= SP) && (geotype[1] >= SP) )
                    ...
                    if (geotype[0] < SP)
                        ...
                        halfl = 0.0;
                    vec1.x = part1->pos.x + part1->dir.x * (halfl + topo->chainparam[part1->chaint].bonddeq) / box.x ;
                    vec1.y = part1->pos.y + part1->dir.y * (halfl + topo->chainparam[part1->chaint].bonddeq) / box.y ;
                    vec1.z = part1->pos.z + part1->dir.z * (halfl + topo->chainparam[part1->chaint].bonddeq) / box.z ;
                    if (geotype[0] < SP)
                        halfl =topo->ia_params[part1->type][part2->type].half_len[0];
                    else
                        halfl = 0.0;
