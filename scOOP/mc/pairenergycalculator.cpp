    ...
    for(i = 0; i < MAXT; i++){
        for(j = 0; j < MAXT; j++){
            ...
            if ( ( (geotype == CHCPSC || geotype == CPSC || geotype == TCHCPSC || geotype == TCPSC) &&
                ...
                intFCE[i][j] = &PairEnergyCalculator::ePscCpsc;
            }
            if ( (geotype == CHCPSC || geotype == CPSC || geotype == TCHCPSC || geotype == TCPSC) &&
                    (other_geotype == CHCPSC || other_geotype == CPSC || other_geotype == TCHCPSC || other_geotype == TCPSC) ){
...
double PairEnergyCalculator::eattractivePscPsc(int patchnum1, int patchnum2) { //patchnum1/2 define if partice 1/2 have two patches
    ...
    vec1 = vec1.perpProject(part1->dir);
    vec1.normalise();
    ...
    vec1 = -1.0 * vec_intrs;
    //vec1=vecScale(vec_mindist,1.0);
    vec1 = vec1.perpProject(part2->dir);
    vec1.normalise();
...
double PairEnergyCalculator::eattractivePscSpa(int patchnum1) {
    ...
    if (topo.ia_params[part1->type][part2->type].geotype[0] < SP) {
        which = 0;
        vec1 = distvec.perpProject(part1->dir);
        vec1.normalise();
        a = DOT(vec1, part1->patchdir[patchnum1]);
        halfl=topo.ia_params[part1->type][part2->type].half_len[0];
    } else {
        which = 1;
        vec1 = distvec.perpProject(part2->dir);
        vec1.normalise();
...
double PairEnergyCalculator::eRepulsive() {
    ...
    if (dist >topo.ia_params[part1->type][part2->type].rcutwca) repenergy = 0.0;
            ...
            repenergy = topo.ia_params[part1->type][part2->type].len[0]*topo.ia_params[part1->type][part2->type].epsilon*(4*en6*(en6-1) + 1.0);
