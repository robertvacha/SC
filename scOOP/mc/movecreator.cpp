...
double MoveCreator::chainDisplace(long target) {
    ...
    dr = ranvec();
    dr.x *= sim->chainm[conf->particleStore[target]->chaint].mx/conf->box.x;
    dr.y *= sim->chainm[conf->particleStore[target]->chaint].mx/conf->box.y;
    dr.z *= sim->chainm[conf->particleStore[target]->chaint].mx/conf->box.z;
    i=0;
    if ( ((sim->wlm[0] == 3)||(sim->wlm[1] == 3)) && (target == 0) ) {
        dr.z = 0;
        dr.y = 0;
        ...
        for (wli=0;wli<sim->wl.wlmdim;wli++) {
            switch (sim->wlm[wli]) {
                case 1: origsyscm = conf->syscm;
                    ...
                    break;
                case 2: sim->wl.origmesh = sim->wl.mesh;
                    sim->wl.neworder[wli] = meshOrderMoveChain(topo->chainlist[target], &sim->wl.mesh, conf->particleStore.size(), chorig,wli);
                    break;
                case 4:
                    sim->wl.neworder[wli] = sim->wl.twoPartDist(wli);
                    break;
                ...
                case 6:
                    ...
                    if ( target == 0 )
                      ...
                      sim->wl.neworder[wli] = radiusholeOrderMoveChain(topo->chainlist[target], chorig,wli,&(conf->particleStore[0]->pos));
                    break;
                case 7:
                    sim->wl.partincontactold = sim->wl.partincontact;
                    if ( target == 0 )
                        ...
                        sim->wl.neworder[wli] = contParticlesMoveChain(topo->chainlist[target],chorig,wli);
                    break;
                default:
                    sim->wl.neworder[wli] = sim->wl.currorder[wli];
                    break;
    ...
    if ( reject || moveTry(energy, enermove, sim->temper) ) {  /* probability acceptance */
        ...
        sim->chainm[conf->particleStore[target]->chaint].rej++;
        if ( (sim->wlm[0] == 1) || (sim->wlm[0] == 5) || (sim->wlm[1] == 1) || (sim->wlm[1] == 5) )
            conf->syscm = origsyscm;
        sim->wl.reject(radiusholemax_orig, sim->wlm);
    } else { /* move was accepted */
        sim->chainm[conf->particleStore[target]->chaint].acc++;
        sim->wl.accept(sim->wlm[0]);
        edriftchanges = enermove - energy + wlener;
    }
...
double MoveCreator::chainRotate(long target) {
    ...
    clusterRotate(target, cluscm, sim->chainr[conf->particleStore[target]->chaint].angle);
    enermove=0.0;
    ...
    if ( reject || moveTry(energy, enermove, sim->temper) ) { /* probability acceptance */
        ...
        sim->chainr[conf->particleStore[target]->chaint].rej++;
        sim->wl.reject(radiusholemax_orig, sim->wlm);
    } else { /* move was accepted */
        sim->chainr[conf->particleStore[target]->chaint].acc++;
        sim->wl.accept(sim->wlm[0]);
        edriftchanges = enermove - energy + wlener;
    }
...
/*				printf("send data: rank: %d\n",sim->mpirank);
                ...
                printf("part0  chaint %ld chainn %ld type %d\n",conf->particleStore[0]->chaint,conf->particleStore[0]->chainn,conf->particleStore[0]->type);
*/
...
double MoveCreator::muVTMove() {
    ...
    int molType = 0;
    for(int i=0; i<topo->molTypeCount; i++) {
    ...
    if(ran2() > 0.5) { ///  insert move
        ...
        if(topo->chainparam[molType].particleTypes[1] == -1) {
            newPart->pos = ranvec();
            newPart->dir = ranvec();
            ...
            if( (conf->sysvolume / (conf->particleStore.size()+1)) *
                ...
                try{
                    conf->particleStore.push_back(newPart);
                } catch(std::bad_alloc& bad) {
                    fprintf(stderr, "\nCONF ERROR: Could not allocate memory for inserting particle");
                    exit(1);
                }
        ...
        target = ran2() * (double)conf->particleStore.size()-0.000001;

        /// is target single-particle Molecular type
        if(topo->chainparam[conf->particleStore[target]->chaint].particleTypes[1] == -1) {
            // do energy calc
            energy = (*calcEnergy)(conf->particleStore[target],1,0);
            ...
            if( ((double)conf->particleStore.size() / conf->sysvolume) *
                ...
                delete conf->particleStore[target]->neighborID;
                delete conf->particleStore[target];

                conf->particleStore[target] = conf->particleStore.back();
                conf->particleStore.pop_back(); // this wont run destructor
